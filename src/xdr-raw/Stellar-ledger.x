...
enum LedgerEntryKeyType // compability struct
{
...
struct LedgerKeyAccount // compability struct
{
    AccountID accountID;
};

struct LedgerKeyTrustLine // compability struct
{
         AccountID accountID;
         Asset asset;
};

struct LedgerKeyTrustOffer // compability struct
{
         AccountID sellerID;
         uint64 offerID;
};

struct LedgerKeyTrustData // compability struct
{
         AccountID accountID;
         string64 dataName;
};

union LedgerKey switch (LedgerEntryKeyType type)
{
case LEDGER_ENTRY_KEY_TYPE_ACCOUNT:
   LedgerKeyAccount account;
case LEDGER_ENTRY_KEY_TYPE_TRUSTLINE:
   LedgerKeyTrustLine trustLine;
case LEDGER_ENTRY_KEY_TYPE_OFFER:
    LedgerKeyTrustOffer offer;
case LEDGER_ENTRY_KEY_TYPE_DATA:
    LedgerKeyTrustData data;
};
